...
/**
 * JEPA preprocessing for the native QNN vision path: fixed 256x256 square
 * resize, CHW FP32, ImageNet normalization.
 */
class JepaImageProcessor : NativeImageProcessor {
    companion object {
    ...
    override fun preprocessNative(image: Bitmap): NativeCausalLm.MultimodalInput {
        ...
        for (i in 0 until pixelCount) {
            val pixel = pixels[i]
            out[i] = ((Color.red(pixel) / 255.0f) - MEAN[0]) / STD[0]
            out[pixelCount + i] =
                ((Color.green(pixel) / 255.0f) - MEAN[1]) / STD[1]
            out[2 * pixelCount + i] =
                ((Color.blue(pixel) / 255.0f) - MEAN[2]) / STD[2]
        }
